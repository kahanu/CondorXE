...
{
    public class StandardWithBaseClassesServiceLayerObjects : RenderBase, IServiceObjects
    {
        #region ctors
        private readonly RequestContext _context;

        public StandardWithBaseClassesServiceLayerObjects(RequestContext context)
        ...
        public void Render()
        {
            ...
            _context.FileList.Add("Generated Standard Service Layer interfaces: ");
            ...
            _context.Dialog.InitDialog(_context.Database.Tables.Count);
            _context.FileList.Add("");
            _context.FileList.Add("Generated Standard Service Layer Base classes: ");
            foreach (string tableName in _script.Tables)
            {
            ...
            _context.Dialog.InitDialog(_context.Database.Tables.Count);
            _context.FileList.Add("");
            _context.FileList.Add("Generated Standard Service Layer classes: ");
            foreach (string tableName in _script.Tables)
            {
...
_output.autoTabLn("");
    ...
    _output.autoTabLn("}");
    _output.autoTabLn("");
    _output.autoTabLn("public " + _context.Utility.BuildModelClassWithNameSpace(StringFormatter.CleanUpClassName(table.Name)) + " GetById(" + _context.Utility.RenderConcreteMethodParameters(table) + ")");
    _output.autoTabLn("{");
        _output.tabLevel++;
        _output.autoTabLn("return " + StringFormatter.CamelCasing(StringFormatter.CleanUpClassName(table.Name)) + _script.Settings.DataOptions.ClassSuffix.Name + ".GetById(" + _context.Utility.RenderCallingMethodParameters(table) + ");");
    _output.tabLevel--;
    _output.autoTabLn("}");
_output.tabLevel--;
_output.autoTabLn("");
